...
# We need a collection for the package information for the grid and standard HTML exports
$MsiPackages = New-Object System.Collections.Generic.List[MsiPackage]

# PSCustomObject did not work with the grids for some reason
#$MsiPackages = New-Object System.Collections.Generic.List[PSCustomObject]
...
# Save standard Powershell HTML export file - same approach as above, just dumping whole package list
$MsiPackages | ConvertTo-Html > "C:\Users\glytz\Desktop\convert.html"
...
#$filename = "MsiInfo_$((Get-Date).Day).$((Get-Date).Month)(month).$((Get-Date).Year)_$((Get-Date).Hour)-$((Get-Date).Minute)-$((Get-Date).Second).html"
...
# Finally: Show editable DataDisplayGrid with MSI package information as well
Add-Type -AssemblyName System.Windows.Forms
Add-Type -AssemblyName System.Drawing

$form = New-Object System.Windows.Forms.Form
$form.Text = "Editable Data Grid"
$form.Size = New-Object System.Drawing.Size(800, 600)

$dataGridView = New-Object System.Windows.Forms.DataGridView
$dataGridView.Dock = [System.Windows.Forms.DockStyle]::Fill
$dataGridView.AllowUserToAddRows = $true # Allow adding new rows
$dataGridView.AllowUserToDeleteRows = $true # Allow deleting rows
$dataGridView.ReadOnly = $false # Make cells editable
...
$form.Controls.Add($dataGridView)
    ...
    $saveButton.Add_Click({
        ...
        # For example, export to CSV:
        # $dataGridView.DataSource | Export-Csv -Path "C:\temp\edited_data.csv" -NoTypeInformation
        Write-Host "Save button clicked. Implement your save logic."
    })
    $form.Controls.Add($saveButton)

$dataGridView.AutoGenerateColumns = $true
$dataGridView.DataSource = $MsiPackages
$form.ShowDialog()
...
# throw "This is a custom terminating error message."
#$tmp += "$p, $productname, $versionstring, $packagecode, $productcode, $upgradecode, $allupgrades, $assignment, $lcid`r`n"
#$MsiPackage = [MsiPackage]@{Number=$p;ProductMame=$productname;Version=$versionstring;PackageCode=$packagecode}
#$CustomObject = [PSCustomObject]@{Number=$p;ProductMame=$productname;Version=$versionstring;PackageCode=$packagecode}
#$MsiPackages.Add($CustomObject)
#$tmp = ""
#$CustomObject
#$MsiPackages.Add([MsiPackage]@{Counter=$p;ProductName=$productname;Version=$versionstring;PackageCode=$packagecode;ProductCode=$productcode;UpgradeCode=$upgradecode;RelatedProductCodes=$allupgrades;Scope=$assignment;Lcid=$lcid})
